...
def map_findings(
    ...
    for finding in findings:
        ...
        if not file or not line:
            print(
                "Finding has no valid file/line; "
...
def format_outside_scope_findings(
    ...
    body = (
        ...
        "The following test coverage findings are relevant to the "
...
def main():
    ...
    review_path = Path(
        "review-context/test-review.json"
    )

    if not review_path.exists():
        print(
            "test-review.json not found"
        )
        return
    ...
    if result.returncode != 0:
        ...
        print(
            "Published summary-only test review "
        ...
        print(
            ...
            "test review inline comments."
        )

        if outside_scope:
            print(
                f"Published {len(outside_scope)} "
                "test findings in the top-level review."
            )
